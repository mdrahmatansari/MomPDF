...
class PDFForm {
    ...
    getXFASignatures() {
        ...
        if (!this.hasXFA())
            return result;
        try {
            const packet = (0, xfa_1.readXfaTemplatePacket)(this.acroForm.dict);
            if (!packet)
                return result;
            const { signatures, manifests } = (0, xfa_1.collectXfaSignatures)((0, xfa_1.parseXfaTemplate)(packet.xml));
            for (const sig of signatures) {
                const refs = sig.manifestUse && manifests.has(sig.manifestUse)
                    ? manifests.get(sig.manifestUse)
                    : sig.inlineRefs;
                result.push({
                    field: sig.field,
                    manifest: (_a = sig.manifestUse) !== null && _a !== void 0 ? _a : null,
                    refs,
                });
            }
        }
    ...
    getXFAJavaScripts() {
        ...
        try {
            ...
            if (!packet)
                return scripts;
            for (const entry of (0, xfa_1.collectXfaScripts)((0, xfa_1.parseXfaTemplate)(packet.xml))) {
                const scriptContent = (_a = entry.scriptNode.text) === null || _a === void 0 ? void 0 : _a.trim();
                if (scriptContent) {
                    scripts.push({
                        field: entry.field,
                        event: entry.event,
            ...
            if (error instanceof Error) {
                if (error.message.includes('decode')) {
                    throw new Error(`Failed to decode XFA template stream: Invalid UTF-8 encoding. ${error.message}`);
                }
                throw new Error(`Failed to parse XFA template: ${error.message}`);
            }
            throw error;
    ...
    setXFAJavaScript(fieldName, eventName, newScript) {
        let packet;
        try {
            packet = (0, xfa_1.readXfaTemplatePacket)(this.acroForm.dict);
        }
        catch (error) {
            throw new Error(`Failed to decode XFA template stream: ${error instanceof Error ? error.message : String(error)}`);
        }
        if (!packet) {
            throw new Error('XFA form not found in document. Ensure the document has XFA forms and was loaded with preserveXFA: true.');
        }
        const doc = (0, xfa_1.parseXfaTemplate)(packet.xml);
        const entries = (0, xfa_1.collectXfaScripts)(doc).filter((e) => e.field === fieldName && e.event === eventName);
        if (entries.length === 0) {
            throw new Error(`Script not found for field "${fieldName}" and event "${eventName}". ` +
                'Verify the field and event names exist in the XFA template.');
        }
        for (const entry of entries) {
            entry.scriptNode.set_content((0, html_entities_1.encode)(newScript));
        }
        const context = this.acroForm.dict.context;
        const newXmlBytes = new TextEncoder().encode(doc.toString());
        const filter = packet.stream.dict.get(core_1.PDFName.of('Filter'));
        const newStream = filter !== undefined
    ...
    flattenWidgetOntoPage(page, widget, appearanceRef) {
        const rectangle = widget.getRectangle();
        const xObjectKey = page.node.newXObject('FlatWidget', appearanceRef);
        page.pushOperators((0, operators_1.pushGraphicsState)(), (0, operators_1.translate)(rectangle.x, rectangle.y), ...(0, operations_1.rotateInPlace)(Object.assign(Object.assign({}, rectangle), { rotation: 0 })), (0, operators_1.drawObject)(xObjectKey), (0, operators_1.popGraphicsState)());
    }
    /**
    ...
    flattenOrphanWidgets() {
        ...
        for (let p = 0, pageCount = pages.length; p < pageCount; p++) {
            ...
            if (!annots)
                continue;
            const annotsToRemove = [];
            for (let i = 0, len = annots.size(); i < len; i++) {
                const annotRef = annots.get(i);
                if (!(annotRef instanceof core_1.PDFRef))
                    continue;
                const dict = this.doc.context.lookup(annotRef);
                if (!(dict instanceof core_1.PDFDict))
                    continue;
                if (dict.lookup(core_1.PDFName.of('Subtype')) !== core_1.PDFName.of('Widget')) {
                    continue;
                }
                // Self-contained field widget (kids inherit /FT from Parent)
                if (!dict.has(core_1.PDFName.of('FT')))
                    continue;
                try {
                    const widget = core_1.PDFWidgetAnnotation.fromDict(dict);
    ...
    findWidgetAppearanceRef(field, widget) {
        ...
        if (field instanceof PDFCheckBox_1.default || field instanceof PDFRadioGroup_1.default) {
            if (refOrDict instanceof core_1.PDFRef) {
                refOrDict = this.doc.context.lookup(refOrDict, core_1.PDFDict);
            }
    ...
    findOrphanWidgetAppearanceRef(widget) {
        var _a, _b, _c, _d;
        const n = (_a = widget.AP()) === null || _a === void 0 ? void 0 : _a.get(core_1.PDFName.of('N'));
        let refOrDict = n;
        if (refOrDict instanceof core_1.PDFRef) {
            const lookedUp = this.doc.context.lookup(refOrDict);
            if (lookedUp instanceof core_1.PDFDict)
                refOrDict = lookedUp;
        }
        if (refOrDict instanceof core_1.PDFDict) {
            const value = widget.dict.lookup(core_1.PDFName.of('V'));
            const state = (_c = (_b = widget.getAppearanceState()) !== null && _b !== void 0 ? _b : (value instanceof core_1.PDFName ? value : undefined)) !== null && _c !== void 0 ? _c : core_1.PDFName.of('Off');
            const ref = (_d = refOrDict.get(state)) !== null && _d !== void 0 ? _d : refOrDict.get(core_1.PDFName.of('Off'));
            if (!(ref instanceof core_1.PDFRef)) {
